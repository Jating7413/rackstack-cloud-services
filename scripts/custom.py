...
if os == "centos":	
	...
	fil.close()
	if a == "php":
		commands.getoutput("echo 'RUN yum install php -y' | sudo cat >> /custom/Dockerfile")
	if b == "mysql":
		commands.getoutput("echo 'RUN yum install mysql -y' | sudo cat >> /custom/Dockerfile")
	if c == "net-tools":
                commands.getoutput("echo 'RUN yum install net-tools -y' | sudo cat >> /custom/Dockerfile")
	if d == "apache":
                commands.getoutput("echo 'RUN yum install httpd -y' | sudo cat >> /custom/Dockerfile")
	if f == "docker":
                commands.getoutput("echo 'RUN yum install docker-engine -y' | sudo cat >> /custom/Dockerfile")
	if g == "python":
                commands.getoutput("echo 'RUN yum install python -y' | sudo cat >> /custom/Dockerfile")
	...
	if launchStatus[0]==0:
		...
		print "Your custom instance is launched successfully"
		print "<br><br>"
...
if os == "ubuntu":	
	...
	fil.close()
	if a == "php":
		commands.getoutput("echo 'RUN sudo apt-get install php7.0 -y' | sudo cat >> /custom/Dockerfile")
	if b == "mysql":
		commands.getoutput("echo 'RUN sudo apt-get install mysql-server -y' | sudo cat >> /custom/Dockerfile")
	if c == "net-tools":
...
if os == "debian":	
	...
	entry = "RUN echo 'root:{}' | chpasswd\n".format(password)
	commands.getoutput("echo 'FROM debian:latest' | sudo cat >> /custom/Dockerfile")
	fil = open("/custom/Dockerfile" , 'a')
	fil.write(entry)
	fil.close()
	if a == "php":
...
if os == "redhat":
        commands.getstatusoutput("sudo cat > /custom/Dockerfile")
        entry = "RUN echo 'root:{}' | chpasswd\n".format(password)
        commands.getoutput("echo 'FROM richxsl/rhel7:latest' | sudo cat >> /custom/Dockerfile")
        fil = open("/custom/Dockerfile" , 'a')
        fil.write(entry)
        fil.close()
        if a == "php":
